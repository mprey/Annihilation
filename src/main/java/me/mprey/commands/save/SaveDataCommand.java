...
public class SaveDataCommand extends BaseCommand {
    ...
    public SaveDataCommand(Annihilation instance) {
        super(instance);
    }
    ...
    public String getName() {
        return Annihilation._l("commands.save.data.name");
    }

    public String getDescription() {
        return Annihilation._l("commands.save.data.description");
    }
    ...
    public boolean execute(CommandSender sender, ArrayList<String> args) {
        if (args.get(0).equalsIgnoreCase("lobby")) {
            if (getInstance().getMapManager().lobbyLoaded()) {
                if (getInstance().getMapManager().getLobby().saveData()) {
            ...
            if (map != null) {
                MapErrorCode error = getInstance().getMapManager().saveMapData(map);
    ...
    public String getUsage() {
        return Annihilation._l("commands.save.data.usage");
    }
}
